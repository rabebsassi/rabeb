...
int main() {
	...
	RenderWindow window(VideoMode(1000, 600), "Rabeb");
	...
	sounds.hopBuffer.loadFromFile("./audio/piw.wav");
	sounds.dishkBuffer.loadFromFile("./audio/fail.wav");
	sounds.ching.setBuffer(sounds.chingBuffer);
	sounds.hop.setBuffer(sounds.hopBuffer);
	sounds.dishk.setBuffer(sounds.dishkBuffer);

	// all textures remain in here. player has 3 textures, which will repeadetly draw, creating the illusion of flying.
	struct Textures {
		Texture player[3];
		Texture pipe;
		Texture background;
	...
	textures.player[0].loadFromFile("./images/player1.png");
	textures.player[1].loadFromFile("./images/player2.png");
	textures.player[2].loadFromFile("./images/player3.png");

	// player singleton struct.
	// v = vertical speed
	...
	player.sprite.setPosition(250, 300);
	...
	game.font.loadFromFile("./fonts/player.ttf");
	...
	game.gameover.setPosition(500, 200);
	game.gameover.setScale(2, 2);
	game.pressR.setString("ENZEL 3LA R BECH T3AWED");
	game.pressR.setFont(game.font);
	game.pressR.setFillColor(Color::White);
	game.pressR.setCharacterSize(50);
	game.pressR.setOrigin(game.pressR.getLocalBounds().width / 2, 0);

	game.pressR.setPosition(500, 350);
	game.scoreText.setFont(game.font);
	game.scoreText.setFillColor(Color::White);
	game.scoreText.setCharacterSize(55);
	...
	bool isAlechPlaying = false;
	// main loop
	while (window.isOpen()) {


		// update score
		player.sprite.setTexture(textures.player[1]);
		game.scoreText.setString("SCOREK: " + to_string(game.score));
		game.highscoreText.setString("A9WA SCORE: " + to_string(game.highscore));

		// update player
		float fx = player.sprite.getPosition().x;
		float fy = player.sprite.getPosition().y;
		float fw = 34 * player.sprite.getScale().x;
		float fh = 24 * player.sprite.getScale().y;
		...
		player.sprite.setTexture(textures.player[player.frame]);

		// move player
		if (game.gameState == started) {
			player.sprite.move(0, player.v);
			player.v += 0.5;
		}
		...
		if (game.gameState == started) {
			if (fy < 0) {
				player.sprite.setPosition(250, 0);
				player.v = 0;
			}
			else if (fy > 600) {
				player.v = 0;
		...
		while (window.pollEvent(event)) {
			...
			// flap
				...
				player.sprite.setPosition(250, 250);
				game.score = 0;
				pipes.clear();
			}
		}
